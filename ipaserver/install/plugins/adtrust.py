...
class update_tdo_to_new_layout(Updater):
    ...
    def retrieve_trust_password(self, packed):
        ...
        # This array needs to be converted to UTF-16-LE string
    ...
    def set_krb_principal(self, principals, password, trustdn, flags=None):
        ...
        entry_data = {
            'objectclass':
                ['krbPrincipal', 'krbPrincipalAux', 'krbTicketPolicyAux', 'top'],
        ...
        redacted = self.change_password_template.format(
            ...
            principal=trust_principal)
        logger.info("Updating Kerberos keys for %s with the following "
    ...
    def execute(self, **options):
        ...
        for t_entry in trusts:
            t_dn = t_entry.dn
            logger.info('Processing trust domain object %s', str(t_dn))
            ...
            if direction & TRUST_BIDIRECTIONAL:
