Unit 13: Managing subUIDs and subGIDs in FreeIPA
================================================
...
In this module you will explore how to manage use of centralized identitiy
mappings.
...
In this	module you will	explore	how to manage use of centralized identitiy
mappings. SubUID/GIDs are used to provide an additional layer of security and
resource isolation for users on a system. They allow a single user to have
multiple UIDs and GIDs, each with its own set of permissions and access to
resources. In this workshop, we will cover the following topics:

- What are subUIDs and subGIDs and why are they used?
- How to view a local user's subUIDs and subGIDs
- How to configure subUIDs and subGIDs for a user
- Common uses for subUIDs and subGIDs
- How to centraly manage subUIDs and subGIDs in FreeIPA

What are subUIDs and subGIDs and why are they used?
---------------------------------------------------

In Linux, every user is assigned a unique UID and every group is assigned a unique GID.
These IDs are used to identify users and groups and to control access to resources.
However, in some cases, it may be desirable to have multiple UIDs and GIDs for a single
user. This is where subUIDs and subGIDs come in.

SubUIDs and subGIDs allow a single user to have multiple UIDs and GIDs, each with its own
set of permissions and access to resources. This can be useful in a number of scenarios,
including:

- Providing resource isolation for users: SubUIDs and subGIDs can be used to isolate resources for different applications or tasks, so that a problem with one application does not affect the others.

- Allowing users to run applications with different permissions: SubUIDs and subGIDs can be used to allow users to run applications with different permissions, without giving them full root privileges.

- Improving security: SubUIDs and subGIDs can be used to limit the damage that a compromised user account can do. If an attacker gains access to a user account with a subUID or subGID, they will only have access to the resources associated with that ID, rather than all of the resources associated with the user's primary UID or GID.

SubUIDs and SubGIDs are additional UIDs and GIDs that are associated with a user. They allow a user to temporarily gain access to resources that are owned by another user or group. For example, a user might use a subUID to run a process as another user, or a subGID to access a group-owned resource.

SubUIDs and SubGIDs are stored in the /etc/subuid and /etc/subgid files, respectively. These files contain a list of users and the subUIDs and subGIDs that are associated with them. The format of these files is as follows::
  username:range:offset

  
Now that we know what subUIDs and subGIDs are and why they are used, let's learn how to view a user's subUIDs and subGIDs.

How to view a user's subUIDs and subGIDs
----------------------------------------

To view a user's subUIDs and subGIDs, you can use the awk command to extract the relevant information from the /etc/subuid and /etc/subgid files.

To view a user's subUIDs, use the following command::

  getsubids $USER

This will display the range of subUIDs assigned to the user. For example::

  0: bob 100000 65536

To view a user's subGIDs, use the following command::
  
  getsubids -g $USER

This will display the range of subGIDs assigned to the user::
