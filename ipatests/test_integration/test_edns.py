...
from ipatests.test_integration.test_dns_locations import TestDNSLocations
...
class TestDNSOverTLS(IntegrationTest):
    ...
    def test_uninstall_all(self):
        ...
        self.master.run_command(["ipa-server-install",
                                 "--uninstall", "--ignore-last-of-role", "-U"])
...
class TestDNSLocations_DoT(TestDNSLocations):
    @classmethod
    def install(cls, mh):
        ...
        # Generate CA for DoT for replica with setup_ca=False
        cls.replicas[0].run_command(["openssl", "req", "-newkey", "rsa:2048",
                                     "-nodes", "-keyout",
                                     "/etc/pki/tls/certs/privkey.pem", "-x509",
                                     "-days", "36500", "-out",
                                     "/etc/pki/tls/certs/certificate.pem",
                                     "-subj",
                                     ("/C=ES/ST=Andalucia/L=Sevilla/"
                                      "O=CompanyName/OU=IT/CN={}/"
                                      "emailAddress=email@example.com")
                                    .format(cls.replicas[0].hostname)])
        cls.replicas[0].run_command(["chown", "named:named",
                                     "/etc/pki/tls/certs/privkey.pem",
                                     "/etc/pki/tls/certs/certificate.pem"])

        tasks.install_master(cls.master, extra_args=server_args)
        tasks.install_replica(cls.master, cls.replicas[0],
                              extra_args=server_args + no_ca_args,
                              setup_ca=False, setup_dns=True)
        tasks.install_replica(cls.master, cls.replicas[1],
                              extra_args=server_args, setup_dns=True)
