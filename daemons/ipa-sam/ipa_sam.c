...
#define KRB_PRINC_CREATE_IDENTITY           0x00000004
			  ...
			  uint32_t   create_flags)
{
	...
	/* Add SID and uidNumber/gidNumber.
	 * We are using ipaIDObject and posixAccount objectclasses here
	 * which will force generation of ipaNTSecurityIdentifier as well */
#if 0
	if ((create_flags & KRB_PRINC_CREATE_IDENTITY)) {
		const char *name = NULL;
		name = talloc_asprintf(mem_ctx, "%.*s", strchr(princ, '@')-princ, princ);
		if (name == NULL) {
			DEBUG(1, ("error creating TDO object data!\n"));
			return false;
		}
		smbldap_set_mod(&mods, LDAP_MOD_ADD,
				LDAP_ATTRIBUTE_OBJECTCLASS,
	...
	switch (mod) {
		case SET_PRINC:
			...
			if ((trust_direction & LSA_TRUST_DIRECTION_INBOUND) != 0) {
				...
				ok   = set_krb_princ(ipasam_state, tmp_ctx, princ_r, NULL,
						     pwd_outgoing, trusted_dn, KRB_PRINC_CREATE_DEFAULT);
				...
				ok |= set_krb_princ(ipasam_state, tmp_ctx, r_tdo_alias, princ_r_tdo,
			...
			if ((trust_direction & LSA_TRUST_DIRECTION_OUTBOUND) != 0) {
				...
				ok |= set_krb_princ(ipasam_state, tmp_ctx, princ_l, NULL,
				 ...
				 * POSIX and SMB identities associated with our domain */
				ok |= set_krb_princ(ipasam_state, tmp_ctx, princ_l_tdo, l_tdo_alias,
						    pwd_incoming, trusted_dn,
						    KRB_PRINC_CREATE_DEFAULT | KRB_PRINC_CREATE_IDENTITY);
			}
			if (!ok) {
				goto done;
			}
		...
		case DEL_PRINC:
			...
			ok |= del_krb_princ(ipasam_state, tmp_ctx, princ_r_tdo, trusted_dn);
			if ((trust_direction & LSA_TRUST_DIRECTION_OUTBOUND) != 0) {
				ok |= del_krb_princ(ipasam_state, tmp_ctx, princ_l, trusted_dn);
				ok |= del_krb_princ(ipasam_state, tmp_ctx, princ_l_tdo, trusted_dn);
			}
			if (!ok) {
				goto done;
			}
					  ...
					  const struct pdb_trusted_domain *td)
{
	...
	if (entry == NULL) { /* FIXME: allow password updates here */
		...
		res = set_cross_realm_princs(ipasam_state, td->domain_name, td->netbios_name,
					     ...
					     td->trust_direction);
		memset(trustpw_incoming, 0, strlen(trustpw_incoming));
		memset(trustpw_outgoing, 0, strlen(trustpw_outgoing));
		if (!res) {
