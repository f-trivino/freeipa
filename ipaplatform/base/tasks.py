...
UNBOUND_TLS_CONF = textwrap.dedent("""
    # auto-generated by IPA installer
    server:
        # Enable tls
        tls-system-cert: yes
        ...
        interface: 0.0.0.0
        interface: ::0
        # Reply to all interfaces
        access-control: 0.0.0.0/0 allow
    forward-zone:
        name: "."
        forward-addr: "1.1.1.1#one.one.one.one"
        forward-tls-upstream: yes
    # ftrivino add authorative BIND server as a stub-zone:
    stub-zone:
        name: {searchdomains}
        stub-addr: 127.0.0.1
        stub-first: yes
...
class BaseTaskNamespace:
    ...
    # FTRIVINO
    def configure_unbound_dns_resolver(self, nameservers, searchdomains, *,
                               unbound_enabled=False, fstore=None):
        """Configure global DNS resolver (e.g. /etc/resolv.conf)

        :param nameservers: list of IP addresses
        :param searchdomains: list of search domaons
        :param unbound_enabled: is unbound enabled?
        :param fstore: optional file store for backup
        """
        if unbound_enabled:
            # break circular import
            ...
            confd = os.path.dirname(paths.UNBOUND_TLS_CONF)
            if not os.path.isdir(confd):
                os.mkdir(confd)
                # owned by root, readable by unbound user
                os.chmod(confd, 0o755)
                self.restore_context(confd, force=True)

            cfg = UNBOUND_TLS_CONF.format(
                searchdomains=" ".join(searchdomains)
            )
            with open(paths.UNBOUND_TLS_CONF, "w") as f:
                os.fchmod(f.fileno(), 0o644)
                f.write(cfg)

            self.restore_context(
                paths.UNBOUND_TLS_CONF, force=True
            )

            knownservices["unbound"].reload_or_restart()

    # FTRIVINO
    def unconfigure_unbound_dns_resolver(self, fstore=None):
        """Unconfigure global DNS resolver (e.g. /etc/resolv.conf)
        ...
        if os.path.isfile(paths.UNBOUND_TLS_CONF):
            # break circular import
            from ipaplatform.services import knownservices

            os.unlink(paths.UNBOUND_TLS_CONF)
            knownservices["unbound"].reload_or_restart()
