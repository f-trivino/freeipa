...
class RedHatTaskNamespace(BaseTaskNamespace):
    ...
    def configure_unbound_dns_resolver(self, nameservers, searchdomains, *,
        ...
        super().configure_unbound_dns_resolver(
            nameservers=nameservers,
            searchdomains=searchdomains,
        ...
        if nm_enabled:
            ...
            # man standing wins.
            if unbound_enabled:
                # push DNS configuration to unbound
                dnsprocessing = "unbound"
            else:
                # update /etc/resolv.conf
                dnsprocessing = "default"
        ...
        if not unbound_enabled and not nm_enabled:
            # no NM running, no unbound detected
            # fall back to /etc/resolv.conf
            logger.debug(
                "Neither Network Manager nor unbound are enabled, "
                "write %s directly.", paths.RESOLV_CONF
            )
            cfg = [
                "# auto-generated by IPA installer",
    ...
    def unconfigure_unbound_dns_resolver(self, fstore=None):
        ...
        :param fstore: optional file store for restore
        """
        super().unconfigure_unbound_dns_resolver(fstore=fstore)
        # break circular import
        from ipaplatform.services import knownservices
